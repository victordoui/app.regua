-- Resolve the business tenant for owners and active invited staff without
...
as $$
  select coalesce(
    (
      select ur.barbershop_user_id
      from public.user_roles ur
      join public.profiles p
        on p.id = ur.profile_id
       and p.user_id = ur.barbershop_user_id
       and p.auth_user_id = ur.user_id
      where ur.user_id = auth.uid()
        and ur.barbershop_user_id is not null
        and ur.role in ('admin', 'barbeiro')
        and p.active = true
...
drop policy if exists appointment_services_staff_manage on public.appointment_services;
create policy appointment_services_staff_manage
...
using (
  exists (
    ...
    where a.id = appointment_services.appointment_id
      and a.user_id = public.current_business_id()
  )
)
with check (
  exists (
...
drop policy if exists clients_staff_manage on public.clients;
create policy clients_staff_manage
on public.clients
for all
to authenticated
using (user_id = public.current_business_id())
...
as $$
declare
  _owner uuid := public.current_business_id();
  _duration integer;
  _price numeric;
  _buffer integer := 0;
  _opening time;

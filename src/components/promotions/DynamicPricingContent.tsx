...
const PricingRuleForm = ({ onSubmit, onClose }: { onSubmit: (data: CreatePricingRuleInput) => void; onClose: () => void }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div><label className="text-sm font-medium">Tipo *</label><Select value={formData.rule_type} onValueChange={(v) => setFormData({ ...formData, rule_type: v as CreatePricingRuleInput['rule_type'] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{RULE_TYPES.map(t => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent></Select></div>
      <div className="grid grid-cols-2 gap-4">
        <div><label className="text-sm font-medium">Modificador</label><Select value={formData.price_modifier_type} onValueChange={(v) => setFormData({ ...formData, price_modifier_type: v as CreatePricingRuleInput['price_modifier_type'] })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="percentage">%</SelectItem><SelectItem value="fixed">R$</SelectItem></SelectContent></Select></div>
